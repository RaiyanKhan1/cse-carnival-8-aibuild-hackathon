...
// RETURNS ALL ROOMS WITH THEIR BOOKINGS
async function list() { return RoomsModel.all(); }

// RETURNS ONE ROOM BY ID OR THROWS NOT_FOUND
...
// VALIDATES INPUT AND CREATES A NEW ROOM WITH AN AUTO ID
...
// VALIDATES PATCH FIELDS AND UPDATES A ROOM
...
// DELETES A ROOM OR THROWS NOT_FOUND
async function remove(id) {
  if ((await RoomsModel.remove(id)) === 0) throw NotFound();
  return { ok: true };
}

// VALIDATES THE WINDOW AND APPENDS A BOOKING IF NO OVERLAP EXISTS
async function book(roomId, booking) {
...
// CANCELS A BOOKING BY ID AND RETURNS THE UPDATED ROOM
async function cancel(roomId, bookingId) {
  if ((await RoomsModel.removeBooking(roomId, bookingId)) === 0) throw NotFound();
  return RoomsModel.byId(roomId);
}

// FILTERS AVAILABLE ROOMS BY DATE WINDOW, CAPACITY, AND REQUIRED EQUIPMENT
async function findAvailable({ date, start, end, minCapacity, equipment } = {}) {
...
export const RoomsService = { list, get, create, update, remove, book, cancel, findAvailable };